...
def extract_entities_wih_custom_model(custom_nlp_text):
    ...
    '''
    entities = {}
    # print("Custom_nlp_ents:", custom_nlp_text)
...
def extract_mobile_number(text, custom_regex=None):
    ...
    #     extension)\s*(\d+))?'''
    if not custom_regex:
        mob_num_regex = r'\+?\d{1,3}[\s-]?\(?\d{1,4}\)?[\s-]?\d{1,4}[\s-]?\d{1,4}'
        phone = re.findall(mob_num_regex, text)
        filtered_phone_numbers = [number for number in phone if len(number.replace(' ', '').replace('-', '')) >= 10]
    else:
        phone = re.findall(re.compile(custom_regex), text)
        filtered_phone_numbers = [number for number in phone if len(number.replace(' ', '').replace('-', '')) >= 10]
    if filtered_phone_numbers:
        number = ''.join(filtered_phone_numbers[0])
        return number
